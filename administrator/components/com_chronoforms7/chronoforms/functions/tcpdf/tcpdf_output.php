...
?>
<?php
	$function['file_name'] = basename($function['file_path']);
	
	$config = \G3\L\Obj::getInstance($function['name'], $function);
	...
	}else if(file_exists(\G3\Globals::get('FRONT_PATH').'vendors'.DS.'tcpdf'.DS.'tcpdf.php')){
		require_once(\G3\Globals::get('FRONT_PATH').'vendors'.DS.'tcpdf'.DS.'config/lang/eng.php');
		require_once(\G3\Globals::get('FRONT_PATH').'vendors'.DS.'tcpdf'.DS.'tcpdf.php');
	}else{
	...
	if(!empty($config->get('owner_pass', "")) OR !empty($config->get('user_pass', ""))){
	...
	if(strlen($config->get('pdf_title')) OR strlen($config->get('pdf_header'))){
		$pdf->SetHeaderData(false, 0, $this->Parser->parse($config->get('pdf_title', '')), $this->Parser->parse($config->get('pdf_header', '')));
	}
	
	if($config->get('disable_pdf_header', 0)){
		$pdf->SetPrintHeader(false);
	}
	
	if($config->get('disable_pdf_footer', 0)){
	...
	$PDF_file_name = $this->Parser->parse($config->get('file_name', 'empty_name.pdf'));
	
	$PDF_view = $config->get('pdf_view', 'I');
	if(($PDF_view == 'F') || ($PDF_view == 'FI') || ($PDF_view == 'FD')){
		$PDF_file_path = $this->Parser->parse($config->get('file_path'));
		
		$pdf->Output($PDF_file_path, $PDF_view);
		
		$this->set($function['name'], ['path' => $PDF_file_path]);
		
		$this->debug[$function['name']] = ['path' => $PDF_file_path];
	}else{
