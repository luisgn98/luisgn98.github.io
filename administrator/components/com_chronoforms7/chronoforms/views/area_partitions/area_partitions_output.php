...
<?php
	...
	$view['class'] = 'ui container fluid';
	$view['params']['id'] = 'area_partitions_'.$view['uid'];
	
	echo '<div class="'.$view['class'].' partitioned" data-sequential="'.$view['sequential'].'" id="'.$view['params']['id'].'">';
	if(!empty($view['areas'])){
		$areas = $view['areas'];
		...
		foreach($items as $key => $item){
			...
			foreach($areas as $k => $section){
				...
				echo $this->Parser->section($view['uid'].'/'.$section['name']);
				echo '</div>';
			}
		}
		if(strpos($view['style'], 'vertical') !== false){
