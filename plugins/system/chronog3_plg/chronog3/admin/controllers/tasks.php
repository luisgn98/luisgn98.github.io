	...
	function validate(){
		...
		if(!empty($this->data['license_key']) OR !empty($this->data['order_number'])){
			...
			if(!empty($this->data['license_key'])){
				$target_url = 'http://www.chronoengine.com/index.php?option=com_chronocontact&task=extra&chronoformname=validateLicense&ver=7&api=4';
				
				if(strpos($this->data['license_key'], '@') !== false){
					$update_fld = explode('@', $this->data['license_key'])[0];
				}
			}else if(!empty($this->data['order_number'])){
				$target_url = 'http://www.chronoengine.com/index.php?option=com_chronocontact&task=extra&chronoformname=validateOrder&ver=7&api=4';
